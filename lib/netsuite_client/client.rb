...
class NetsuiteClient
 ...
 def find_by_contains(klass, name, value)
    basic = constantize(klass).new
    ...
    when 'Fixnum'
      ...
      ref.xmlattr_operator = SearchLongFieldOperator::Contains
    when 'TrueClass' || 'FalseClass'
        ref = basic.send("#{name}=".to_sym, SearchBooleanField.new)
    else
      ref = basic.send("#{name}=".to_sym, SearchStringField.new)
      ref.xmlattr_operator = SearchStringFieldOperator::Contains
    end
